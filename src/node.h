...
class Tree {
 public:
  ...
  void balance_tree(tree_element_<Key, T> *current);  // балансировка дерева
  void balance_tree_head(
      tree_element_<Key, T> *current);  // балансировка дерева относительно head
  void obnul(tree_element_<Key, T> *current);
  void erase(iterator pos);

  int counter(tree_element_<Key, T> *current);  // счетчик элементов
  int counter_left(
      tree_element_<Key, T> *current);  // счетчик элементов слева от head
  int counter_right(
      tree_element_<Key, T> *current);  // счетчик элементов справа от head

  tree_element_<Key, T> *search_value(tree_element_<Key, T> *head,
                                      const Key &key);
  size_type size();  //  возвращает количество элементов
  bool empty();  //  возвращает количество элементов
...
void Tree<Key, T>::insert_into_tree(tree_element_<Key, T> *head,
  ...
  if (current->values.first < head->values.first ||
      (current->values.first == head->values.first &&
       count_ == 0)) {  // идем влево от head
...
void Tree<Key, T>::balance_tree(tree_element_<Key, T> *current) {
  if ((current->left && current->right) ||
      current == head_) {  // есть и левый и правый потомок
    // рекурсивно идем вниз по дереву
    if (current->left) balance_tree(current->left);
    if (current->right) balance_tree(current->right);
  } else if (current->left) {  // есть только левый потомок
    if (current->left->left ||
        current->left->right) {  // у него тоже есть потомки
      if (current->back->right == current) {  // меняем элементы местами
  ...
  } else if (current->right) {  // есть только правый потомок
    if (current->right->left ||
        current->right->right) {  // у него тоже есть потомки
      if (current->back->left == current) {  // меняем элементы местами
...
void Tree<Key, T>::balance_tree_head(tree_element_<Key, T> *current) {
  if ((counter_left(current) - counter_right(current)) <
      -2) {  // если правая часть больше левой относительно current
    if (head_ != current) {
    ...
    insert_into_tree(head_, current);
  } else if ((counter_left(current) - counter_right(current)) >
             2) {  // если левая часть больше правой относительно current
    if (head_ != current) {
...
tree_element_<Key, T> *Tree<Key, T>::search_value(tree_element_<Key, T> *head,
                                                  const Key &key) {
  if (key < head->values.first) {  // идем влево от head
    if (head->left) head = search_value(head->left, key);
  }
  if (key > head->values.first) {  // идем вправо от head
